...
#define  EX_PHYSICS_TASK_PRIO       15u
...
#define  EX_MOVEMENT_TASK_PRIO              22u
...
static OS_FLAG_GRP 	sd_flag;
static OS_SEM 		sldsem;
...
static volatile uint32_t direction;
...
//				 &err);
	  ...
	  OSTaskCreate(&Ex_LCD_Display_TaskTCB,                          /* Create the Start Task.                               */
	  	  ...
	  	  OSFlagCreate ((OS_FLAG_GRP  *) &sd_flag,
	                      (CPU_CHAR     *)"speed Flag",
	                      (OS_FLAGS)      0,
	                      (RTOS_ERR     *)&err);


	  	  OSSemCreate ((OS_SEM      *)&sldsem,
	                     (CPU_CHAR    *)"SLD Semafore",
	                     (OS_SEM_CTR)   0,
	                     (RTOS_ERR    *)&err);
	  	  ...
	  	  OSMutexCreate ((OS_MUTEX      *)&movementmut,
	                     (CPU_CHAR    *)"Direction Mutex",
	                     (RTOS_ERR    *)&err);

	  	  OSMutexCreate ((OS_MUTEX      *)&gainmut,
	                     (CPU_CHAR    *)"Speed Mutex",
	                     (RTOS_ERR    *)&err);
...
static  void  Ex_Gain_Task (void  *p_arg)
{
    ...
    while (1)
    {
        ...
        OSFlagPost(&sd_flag, 0x1, OS_OPT_POST_FLAG_SET, &err);
    }
}
...
static  void  Ex_Movement_Task (void  *p_arg)
{
    ...
    while (1)
    {
       ...
       if (temp != 0)
       {
    	   OSFlagPost(&sd_flag, 0x2, OS_OPT_POST_FLAG_SET, &err);
       }
